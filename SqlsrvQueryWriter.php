...
class SqlsrvQueryWriter extends AQueryWriter implements QueryWriter
{
...
\R::ext('setupSqlsrv', function ($pdo) {

    $db = new RPDO($pdo);
    $adapter = new DBAdapter($db);

    $writer = new SqlsrvQueryWriter($adapter);
    $redbean = new OODB($writer);

//        $redbean->freeze( ( $frozen === TRUE ) );
    \R::configureFacadeWithToolbox(new ToolBox($redbean, $adapter, $writer));
